...
def run_hybrid_evaluation(env_instance: Type[TemperatureControlEnv],
                          trained_model_path: str,
                          num_episodes: int) -> pd.DataFrame:
    """Executes the hybrid evalation using a combination of the PID and RL
    algorithm.
    ...
    Args:
        ...
        trained_model_path (str): filepath of the trained model

    Returns:
        (pd.DataFrame): dataframe object of the hybrid results.

    Raises:
        None.
    """
    # define & initialize output
    hybrid_evaluation_data = []
    model = PPO.load(trained_model_path, env=env_instance())
    ...
    for each_episode in range(num_episodes):
        ...
        while (not is_terminated and not is_truncated) and (step_counter <
            ...
            action_with_rl_influence, _states = model.predict(observations,
                                                              deterministic=True)
        ...
        hybrid_evaluation_data.extend(current_episode_data)
        env.close()

    return pd.DataFrame(hybrid_evaluation_data)
